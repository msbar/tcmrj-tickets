...
class HomeAsGestorTest(TestCase):
    ...
    def test_username_data_as_suporte(self):
        """Must have username data in template"""
        expected = '<span>{}</span>'.format(self.user.username)
        self.assertContains(self.response, expected)

    def test_group_data_as_suporte(self):
        """Must have group name suporte data in template"""
