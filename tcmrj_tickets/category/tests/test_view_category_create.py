...
class CategoryCreateViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.response = self.client.get(r('category:create'))

    def test_get_as_gestor(self):
        """GET /categoria/create must return status code 200"""
        self.assertEqual(200, self.response.status_code)

    def test_get_as_suporte(self):
        """GET /categoria/create must return status code 403 for group suporte"""
        self.client.force_login(self.user_suporte)
        self.response = self.client.get(r('category:create'))
        self.assertEqual(403, self.response.status_code)

    def test_get_as_padrao(self):
        """GET /categoria/create must return status code 403 for group padrao"""
        self.client.force_login(self.user_suporte)
        self.response = self.client.get(r('category:create'))
        self.assertEqual(403, self.response.status_code)
...
class CategoryCreatePostValid(TestCase):
    ...
    def setUp(self):
        ...
        self.response = self.client.post(r('category:create'), data)
...
class CategoryCreatePostInvalid(TestCase):
    fixtures = ['user.json','group.json']
    def setUp(self):
        self.user = User.objects.get(pk=1)
        self.client.force_login(self.user)

        self.response = self.client.post(r('category:create'), {})
